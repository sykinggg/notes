...
这种类型的简单实现是实用的，但它没有真正展示出装饰能够贡献出来的全部潜能。为这个，我们首先区分一下我的`Coffee`示例和`Freeman`，`Sierra`和`Bates`所著Head First Design Patterns这一本优秀的书中围绕Mackbook商店建立的模型，这两个之间的不同。
...
>
> 这被看做是对原来的`Macbook`对象构造器方法的装饰，它并没有将其重写（例如，`screenSize()`)，我们所定义的`Macbook`的其它属性也保持不变，完好无缺。
>
