### 【进阶7-3期】Throttle 和 Debounce 在 React 中的应用
...
在的 `autocomp` 组件中，一旦在输入框中输入一个单词，它就会请求 `api/users` 获取要显示的用户列表。 在每个字母输入后，触发异步网络请求，并且成功后通过 `this.setState` 更新DOM。
...
`lodash` 也提供了一个 `throttle` 函数，可以在 JS 程序中使用它。
