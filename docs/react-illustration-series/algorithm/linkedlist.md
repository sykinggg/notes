...
在[react 高频对象](../main/object-structure#Fiber)中对`fiber`对象的属性做了说明, 这里列举出 4 个链表属性.
...
在[react 高频对象](../main/object-structure#Hook)中对`Hook`对象的属性做了说明, `Hook`对象具备`.next`属性, 所以`Hook`对象本身就是链表中的一个节点.
