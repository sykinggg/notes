...
本节看看`before mutation阶段`（执行`DOM`操作前）都做了什么。
...
可以看见，`getSnapshotBeforeUpdate`是在`commit阶段`内的`before mutation阶段`调用的，由于`commit阶段`是同步的，所以不会遇到多次调用的问题。
...
经过本节学习，知道了在`before mutation阶段`，会遍历`effectList`，依次执行：
