在[新的React架构一节](/react/preparation/newConstructure)中，提到的**虚拟DOM**在`React`中有个正式的称呼——`Fiber`。在之后的学习中，会逐渐用`Fiber`来取代**React16虚拟DOM**这一称呼。
...
如果你的源码中`fiber.expirationTime`仍存在，请参照[调试源码](react/preparation/source)章节获取最新代
